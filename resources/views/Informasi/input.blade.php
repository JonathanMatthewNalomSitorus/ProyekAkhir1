...
<body class="bg-gray-100">
    ...
    <form action="/informasi/store" method="post" enctype="multipart/form-data" class="bg-white p-6 rounded shadow-md max-w-lg mx-auto my-8">
        @csrf
        <div class="mb-4">
            <label for="id" class="block text-gray-700">ID</label><br>
            <input type="text" name="id" class="border border-gray-300 p-2 w-full">
        </div>

        <div class="mb-4">
            <label for="judul" class="block text-gray-700">Judul</label><br>
            @error('judul')
        ...
        <div class="mb-4">
            <label for="isi" class="block text-gray-700">Isi</label><br>
            @error('isi')
            <div class="alert alert-danger text-red-500">{{$message}}</div>
            @enderror
            <textarea name="isi" cols="30" rows="10" class="border border-gray-300 p-2 w-full"></textarea>
        </div>

        <div class="mb-4">
            <label for="tanggal" class="block text-gray-700">Tanggal Dibuat</label><br>
            @error('tanggal')
            <div class="alert alert-danger text-red-500">{{$message}}</div>
            @enderror
            <input type="date" name="tanggal" class="border border-gray-300 p-2 w-full">
        </div>

        <div class="mb-4">
            <label for="foto" class="block text-gray-700">Foto</label><br>
            @error('foto')
            <div class="alert alert-danger text-red-500">{{$message}}</div>
            @enderror
            <input type="file" name="foto" class="border border-gray-300 p-2 w-full">
        </div>
